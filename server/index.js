...
function handleMessage(client, raw) {
  ...
  if (event.type === "calibrate") {
    enqueueUnityCalibrateEvent(client.roomCode, event.payload);
...
function enqueueUnityCalibrateEvent(roomCode, payload) {
  ensureUnityEventQueue(roomCode);
  const queue = unityEvents.get(roomCode);
  ...
  queue.push({
    ...
    screenFacingForward: Boolean(payload?.screenFacingForward || payload?.screenFacing === "front"),
    timestamp: Number(payload?.timestamp || sample.timestamp || Date.now()),

    accelX: Number(sample.accel?.x || payload?.baselineAccel?.x || 0),
    accelY: Number(sample.accel?.y || payload?.baselineAccel?.y || 0),
    accelZ: Number(sample.accel?.z || payload?.baselineAccel?.z || 0),

    rotationAlpha: Number(sample.rotation?.alpha || 0),
    rotationBeta: Number(sample.rotation?.beta || 0),
    rotationGamma: Number(sample.rotation?.gamma || 0),

    orientationAlpha: Number(sample.orientation?.alpha || payload?.neutralOrientation?.alpha || 0),
    orientationBeta: Number(sample.orientation?.beta || payload?.neutralOrientation?.beta || 0),
    orientationGamma: Number(sample.orientation?.gamma || payload?.neutralOrientation?.gamma || 0),
